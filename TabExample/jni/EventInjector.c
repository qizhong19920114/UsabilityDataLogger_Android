...
//com.tabexample.Events.ScanFiles(Native Method)

jint Java_com_tabexample_Events_intEnableDebug( JNIEnv* env,jobject thiz, jint enable ) {
...
jint Java_com_tabexample_Events_intSendEvent(JNIEnv* env,jobject thiz, jint index, uint16_t type, uint16_t code, int32_t value) {
...
jint Java_com_tabexample_Events_ScanFiles( JNIEnv* env,jobject thiz ) {
...
jstring Java_com_tabexample_Events_getDevPath( JNIEnv* env,jobject thiz, jint index) {
	return (*env)->NewStringUTF(env, pDevs[index].device_path);
}
jstring Java_com_tabexample_Events_getDevName( JNIEnv* env,jobject thiz, jint index) {
	if (pDevs[index].device_name == NULL) return NULL;
	else return (*env)->NewStringUTF(env, pDevs[index].device_name);
}

jint Java_com_tabexample_Events_OpenDev( JNIEnv* env,jobject thiz, jint index ) {
	return open_device(index);
}

jint Java_com_tabexample_Events_RemoveDev( JNIEnv* env,jobject thiz, jint index ) {
	return remove_device(index);
}

jint Java_com_tabexample_Events_PollDev( JNIEnv* env,jobject thiz, jint index ) {
	if (index >= nDevsCount || pDevs[index].ufds.fd == -1) return -1;
...
jint Java_com_tabexample_Events_getType( JNIEnv* env,jobject thiz ) {
	return event.type;
}

jint Java_com_tabexample_Events_getCode( JNIEnv* env,jobject thiz ) {
	return event.code;
}

jint Java_com_tabexample_Events_getValue( JNIEnv* env,jobject thiz ) {
	return event.value;
}
